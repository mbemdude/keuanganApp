    ...
    if ($ext === 'csv') {
        ...
        if ($handle !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                $nama_supplier  = $data[0];
                $no_telp        = $data[1];
                $no_mitra       = $data[2];

                $query = "INSERT INTO barang (nama_supplier, no_telp, no_mitra) VALUES (?, ?, ?)";

                $stmt = $db->prepare($query);
                $stmt->execute([$nama_supplier, $no_telp, $no_mitra]);
            }
            fclose($handle);
        }
        ...
        foreach ($worksheet->getRowIterator() as $rowIndex => $row) {
            ...
            $nama_supplier = $worksheet->getCell("A$rowIndex")->getCalculatedValue();
            $no_telp = $worksheet->getCell("B$rowIndex")->getCalculatedValue();
            $no_mitra = $worksheet->getCell("C$rowIndex")->getCalculatedValue();

            $query = "INSERT INTO barang (nama_supplier, no_telp, no_mitrak) VALUES (?, ?, ?)";
            $stmt = $db->prepare($query);
            $stmt->execute([$nama_supplier, $no_telp, $no_mitra]);
        }
    } else {
