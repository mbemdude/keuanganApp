...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['export'])) {
    ...
    $sheet->setCellValue('D1', 'Nilai UAS');
    ...
    foreach ($data as $row) {
        ...
        $sheet->setCellValue('D' . $rowNumber, $row['nilai_uas']);
    ...
    if ($ext === 'csv') {
        ...
        if ($handle !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                $mapel = trim($data[2]);
                $uas = is_numeric($data[3]) ? floatval($data[3]) : null;
                ...
                if ($existing) {
                    $stmt = $db->prepare("UPDATE nilai_rapor SET nilai_uas=? WHERE id=?");
                    $stmt->execute([$uas, $existing]);
                } else {
                    $stmt = $db->prepare("INSERT INTO nilai_rapor (siswa_id, tahun_ajaran_id, mata_pelajaran_id, nilai_uas, semester) VALUES (?, ?, ?, ?, ?)");
                    $stmt->execute([$siswa_id, $tahun_ajaran_id, $mapel_id, $uas, $semester]);
                }
            }
            fclose($handle);
        }
        ...
        if (in_array($ext, ['xls', 'xlsx'])) {
            ...
            foreach ($sheet->getRowIterator(2) as $row) {
                ...
                $mapel       = $values[2];
                $uas         = is_numeric($values[3]) ? floatval($values[3]) : null;
                $semester    = $values[4];
            ...
            <div class="card mx-3">
                ...
                <div class="card-body">
                    <ul>
                        ...
                        <a href="assets/sample/Sample Import Nilai UAS.xlsx" class="btn btn-primary">Download Sample</a>
                    </ul>
                </div>
            </div>
        </div>
