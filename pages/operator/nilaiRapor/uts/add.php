...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['simpan_nilai'])) {
    ...
    foreach ($_POST['siswa_id'] as $key => $siswa_id) {
        $nilai_uts = $_POST['nilai_uts'][$key] ?? null;

        $query = "INSERT INTO nilai_rapor (siswa_id, mata_pelajaran_id, tahun_ajaran_id, semester, nilai_uts) 
                  VALUES (:siswa_id, :mata_pelajaran_id, :tahun_ajaran_id, :semester, :nilai_uts)
                  ON DUPLICATE KEY UPDATE nilai_uts = :nilai_uts";
                    ...
                    <?php if (!empty($_POST['kelas_id']) && !empty($_POST['mata_pelajaran_id']) && !empty($_POST['semester']) && !empty($_POST['tahun_ajaran_id'])) { ?>
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                $stmt = $db->prepare("SELECT s.id AS siswa_id, s.nama, nr.nilai_uts
                                                      FROM siswa s 
